...
module.exports = function MC_EXTRA(registry) {
    ...
    if (CONFIG.integrated === true) {
        ...
        EntryConsumer.addAction(targetedJSONAction("loot", (name) => {
            const namespace = mc.getNamespace();
            return path.resolve(process.cwd(), 'data', namespace.namespace, 'loot_tables', namespace.path + name + ".json")
        }));
        EntryConsumer.addAction(targetedJSONAction("predicate", (name) => {
            const namespace = mc.getNamespace();
        ...
        EntryConsumer.addAction(targetedJSONAction("advancement", (name) => {
            const namespace = mc.getNamespace();
            return path.resolve(process.cwd(), 'data', namespace.namespace, 'advancements', namespace.path + name + ".json")
        }));
        mc.on("start", (e) => {
            state = {
                tags: {}
        ...
        mc.on("end", (e) => {
            for (const name in state.tags) {
                const tag = state.tags[name];
                const file = new File();
                file.setPath(path.resolve(process.cwd(), 'data', tag.namespace.namespace, 'tags', tag.type, tag.namespace.path + name + ".json"));
                file.setContents(JSON.stringify({
                    replace: tag.replace,
        ...
        EntryOp.addAction({
            ...
            exec(file, tokens) {
                const { token } = tokens.shift();
                const [type, name, replace] = token.split(/\s+/);
                const parts = path.win32.normalize(path.relative(
        ...
        EntryOp.addAction(targetedJSONAction("loot", (name, file) => {
            const parts = path.win32.normalize(path.relative(
                path.resolve(process.cwd(), 'src'),
                file.split(".")[0]
            )).split("\\");
            return path.resolve(process.cwd(), 'data', parts[0], 'loot_tables', [...parts.splice(1), name].join("/") + ".json")
        }));
        EntryOp.addAction(targetedJSONAction("predicate", (name, file) => {
            const namespace = mc.getNamespace();
            return path.resolve(process.cwd(), 'data', namespace.namespace, 'predicates', namespace.path + name + ".json")
        }));
        ...
        EntryOp.addAction(targetedJSONAction("advancement", (name, file) => {
            const namespace = mc.getNamespace();
            return path.resolve(process.cwd(), 'data', namespace.namespace, 'advancements', namespace.path + name + ".json")
        }));
